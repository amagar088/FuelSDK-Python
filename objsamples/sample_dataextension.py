...
from MarketingCloudSDK.objects import ET_DataExtension, ET_DataExtension_Row, ET_CreateOptions, ET_DeleteOptions, ET_UpdateOptions, ET_SaveOption, ET_DataExtension_Column
myClient = ET_Client()
...
try:
    ...
    myDEColumn = ET_DataExtension_Column()
    ...
    ### DataExtesnion_Row UPSERT operation
    saveAction = ET_SaveOption("UpdateAdd")
    saveAction.auth_stub = myClient
    # Here Explicitly pssing the parameter RequestType and QueuePriority
    #Must pass the third parameter for UPSERT operation
    updateOptions = ET_UpdateOptions(RequestType, QueuePriority, saveAction.saveOptions)
    dataExtensionRow = ET_DataExtension_Row()
    dataExtensionRow.saveOptions = saveAction
    updateOptions.auth_stub = myClient
    dataExtensionRow.auth_stub = myClient
    dataExtensionRow.CustomerKey = "Synchronous_DataExtension_New"
    dataExtensionRow.props = {"Primary_Key": "Key_Value", "SecondField": "SecondValue", "ThirdField": "ThirdValue"}
    dataExtensionRow.updateOptions = updateOptions
    results = dataExtensionRow.patch()
    print('Upsert Status: ' + str(results.status))
    print('Code: ' + str(results.code))
    print('Message: ' + str(results.message))
    print('Results: ' + str(results.results))
